...
| **Progress** | completed | Feature "Evolução" na aba do aluno. Backend: DELETE /progress-photos/:id (4 testes, DeleteProgressPhotoUseCase). Frontend: 8 hooks (useProgressRecords, useCreateProgressRecord, useUpdateProgressRecord, useDeleteProgressRecord, useProgressPhotos, useRequestPhotoUploadUrl, useSaveProgressPhoto, useDeleteProgressPhoto), 5 componentes (studentProgressSection, progressRecordsTab, progressPhotosTab, progressRecordFormDialog, uploadProgressPhotoDialog). Aba "Evolução" adicionada ao studentDetail.tsx. |
