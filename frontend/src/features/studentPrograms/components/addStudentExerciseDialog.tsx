...
import { useAddStudentExercise } from "@/features/studentPrograms/hooks/useAddStudentExercise"
...
type ParamsValues = z.infer<typeof paramsSchema>

interface AddStudentExerciseDialogProps {
  open: boolean
...
export function AddStudentExerciseDialog({
  open,
  onOpenChange,
  workoutDayId,
  programId,
}: AddStudentExerciseDialogProps) {
  ...
  const addExercise = useAddStudentExercise({
    programId,
    onOpenChange: (open) => {
      if (!open) handleClose()
    },
  ...
  const form = useForm<ParamsValues>({
  ...
  function onSubmit(values: ParamsValues) {
    ...
    addExercise.mutate({
      ...
      data: {
        ...
        repetitions: values.repetitions ? Number(values.repetitions) : undefined,
        plannedWeight: values.plannedWeight || undefined,
        restSeconds: values.restSeconds ? Number(values.restSeconds) : undefined,
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 min-w-0 w-full">
            ...
            <FieldGroup>
              <div className="grid grid-cols-2 gap-4">
                <Field>
                  <FieldLabel htmlFor="s-exercise-sets">Séries *</FieldLabel>
                  <Input
                    id="s-exercise-sets"
                    type="number"
                    min="1"
                ...
                <Field>
                  <FieldLabel htmlFor="s-exercise-repetitions">Repetições</FieldLabel>
                  <Input
                    id="s-exercise-repetitions"
                    type="number"
                    min="1"
                    placeholder="Ex: 12"
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field>
                  <FieldLabel htmlFor="s-exercise-weight">Carga planejada</FieldLabel>
                  <Input
                    id="s-exercise-weight"
                    placeholder="Ex: 20kg"
                    {...form.register("plannedWeight")}
                  />
                </Field>

                <Field>
                  <FieldLabel htmlFor="s-exercise-rest">Descanso (seg)</FieldLabel>
                  <Input
                    id="s-exercise-rest"
                    type="number"
                    min="0"
              ...
              <Field>
                <FieldLabel htmlFor="s-exercise-notes">Observações</FieldLabel>
                <Input
                  id="s-exercise-notes"
