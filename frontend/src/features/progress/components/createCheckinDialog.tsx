...
export function CreateCheckinDialog({
  ...
  const createCheckin = useCreateCheckin(studentId, () => handleClose())
    ...
    try {
      const uploadedPhotos = await Promise.all(
        photos.map(async (p) => {
          ...
          await progressService.uploadToS3(uploadUrl, p.file)
      ...
      const message = axios.isAxiosError(err)
        ? (err.response?.data?.message ?? "Erro ao fazer upload das fotos")
