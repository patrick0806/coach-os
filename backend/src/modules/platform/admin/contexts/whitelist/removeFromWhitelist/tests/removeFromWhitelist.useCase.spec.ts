...
describe("RemoveFromWhitelistUseCase", () => {
  let useCase: RemoveFromWhitelistUseCase;
  let personalsRepository: ReturnType<typeof makePersonalsRepository>;

  beforeEach(() => {
    personalsRepository = makePersonalsRepository();
    useCase = new RemoveFromWhitelistUseCase(personalsRepository as any);
  });

  it("should remove personal from whitelist", async () => {
    await useCase.execute("personal-1");
    expect(personalsRepository.setWhitelisted).toHaveBeenCalledWith("personal-1", false);
  });
