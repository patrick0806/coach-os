...
const makeRepository = () => ({
  create: vi.fn().mockResolvedValue(makeWorkingHours()),
...
describe("CreateWorkingHoursUseCase", () => {
  ...
  beforeEach(() => {
    ...
    useCase = new CreateWorkingHoursUseCase(repository as any);
  });

  it("should create working hours successfully", async () => {
  ...
  it("should throw validation error for invalid effectiveFrom format", async () => {
    await expect(
      useCase.execute(
        {
          dayOfWeek: 1,
  ...
  it("should throw ConflictException when overlapping entry exists", async () => {
    repository.findByDayOfWeek.mockResolvedValue([
      makeWorkingHours({ startTime: "10:00", endTime: "14:00" }),
    ]);
